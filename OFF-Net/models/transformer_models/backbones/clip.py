...
__all__ = ["freeze", "get_proj", "get_swin_b", "SensorNaiveTFEncoder", "get_swin_t", "get_swin_s", "CLIP"]
...
class CLIP(nn.Module):
    def __init__(self, enc_in=27, d_model=128, n_head=128, max_seq_length=240, num_layers=6, dim_feedforward=128,
                 out_features=128, scale=True, action_vision=False, vision_encoder="swin_t"):
        """
        :param enc_in: 输入传感器维度
        :param d_model: 传感器tokenize之后TFencoder处理的维度
        :param n_head:
        :param max_seq_length: 最大传感器长度，不足则padding
        :param num_layers: encoder block层数
        :param dim_feedforward: 前向传播维度
        :param scale: 对比的温度参数
        :param action_vision: action和视觉是否一起encode
        :param get_vision_encoder: vision_encoder的获取函数，默认swin_b
        """
    ...
    def forward(self, vision: torch.Tensor, sensor: torch.Tensor):
        vision, vf, af = self.encode_vision(vision, sensor)
        sensor, sf = self.encode_sensor(sensor)

        logit_scale = torch.exp(self.logit_scale)
        logit = vision @ sensor.t()
        if self.scale:
            logit = logit_scale * logit

        return logit, vf, sf, af  # logit[a][b], dot product of vision[a] & sensor[b]
...
if __name__ == '__main__':
    ...
    stem = SensorConvStem(enc_in=enc_in, max_seq_length=max_seq_length, d_model=d_model)
    test_data = torch.randn((2, seq_length, enc_in))
    test_output = stem(test_data)

    s_encoder = SensorNaiveTFEncoder(n_head=n_head)
    test_output2 = s_encoder(test_data)  # batch_size, d_model
